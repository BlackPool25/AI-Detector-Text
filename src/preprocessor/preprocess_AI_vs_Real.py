...
def detect_schema(csv_path: str, sample_rows: int = 1000) -> SchemaInfo:
    """
    Auto-detect schema from CSV file using fuzzy column matching.
    ...
    # Attempt to read a small sample using multiple encodings
    ...
    label_patterns = r'^(label|class|is_ai|category|type|generated)$'
    model_patterns = r'^(model|generator|llm|model_name)$'
    domain_patterns = r'^(domain|genre|task|topic)$'
    ...
    # SPECIAL CASE: headerless or unconventional CSVs where first column is label (0/1)
    try:
        if detected['label_column'] is None and df_sample.shape[1] >= 2:
            first_col_vals = df_sample.iloc[:, 0].dropna().unique().tolist()
            if set(str(x) for x in first_col_vals).issubset({'0', '1', 0, 1}):
                logging.info("Detected first column appears to be binary labels (0/1). Using first column as label and second as text.")
                detected['label_column'] = df_sample.columns[0]
                if detected['text_column'] is None:
                    detected['text_column'] = df_sample.columns[1]
                detected_label_format = 'binary_numeric'
            else:
    ...
    if detected['label_column'] is None:
        # As a fallback, look for numeric 0/1 anywhere
        for col in columns:
            try:
                vals = df_sample[col].dropna().unique()
                if set(str(x) for x in vals).issubset({'0', '1', '0.0', '1.0', 0, 1}):
                    detected['label_column'] = col
                    detected_label_format = 'binary_numeric'
                    break
            except Exception:
    ...
    if detected['label_column'] is None:
        raise ValueError(
            f"Could not detect label column. Available columns: {columns}\n"
            f"Please ensure your CSV has a column matching: {label_patterns}"
        )

    # Analyze label format
    label_col = detected['label_column']
    unique_labels = df_sample[label_col].dropna().unique().tolist()
    label_format = detected_label_format if 'detected_label_format' in locals() and detected_label_format else "unknown"

    if label_format == "unknown":
        # Check for numeric 0/1 including floats like 0.0 and 1.0
        try:
            numeric_labels = set(pd.to_numeric(pd.Series(unique_labels), errors='coerce').dropna())
            if numeric_labels.issubset({0, 1, 0.0, 1.0}):
                label_format = "binary_numeric"
        except:
            pass
        
        if label_format == "unknown":
            if set(str(x) for x in unique_labels).issubset({'0', '1', 0, 1, '0.0', '1.0'}):
                label_format = "binary_numeric"
...
def normalize_labels(df: pd.DataFrame, label_col: str, schema_info: SchemaInfo) -> pd.DataFrame:
    """
...
def clean_chunk(df: pd.DataFrame, schema_info: SchemaInfo, config: ProcessingConfig) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    """
    ...
    # Normalize labels
    df = normalize_labels(df, schema_info.label_column, schema_info)
...
def segregate_and_save(
    ...
    # Save human data: real/RAID-Dataset/human/
    if len(df_human) > 0:
        human_dir = output_dir / 'real' / 'RAID-Dataset' / 'human'
        ...
        # Also save a copy into AI-Vs-Real-Dataset under real (non-destructive)
        avr_human_dir = output_dir / 'real' / 'AI-Vs-Real-Dataset'
        avr_human_dir.mkdir(parents=True, exist_ok=True)
        timestamp = datetime.now().strftime('%Y%m%dT%H%M%S')
        avr_human_file = avr_human_dir / f'chunk_{chunk_num:04d}_{timestamp}.parquet'
        df_human.to_parquet(avr_human_file, compression=compression, index=False, engine='pyarrow')
    
    # Save AI data by model: ai/RAID-Dataset/{model_name}/
    if len(df_ai) > 0:
        for model_name, model_df in df_ai.groupby('model'):
            ...
            model_dir = output_dir / 'ai' / 'RAID-Dataset' / safe_model_name
            ...
            # Also save a copy into AI-Vs-Real-Dataset under ai (non-destructive)
            avr_model_dir = output_dir / 'ai' / 'AI-Vs-Real-Dataset' / safe_model_name
            avr_model_dir.mkdir(parents=True, exist_ok=True)
            timestamp = datetime.now().strftime('%Y%m%dT%H%M%S')
            avr_model_file = avr_model_dir / f'chunk_{chunk_num:04d}_{timestamp}.parquet'
            model_df.to_parquet(avr_model_file, compression=compression, index=False, engine='pyarrow')
    
    # Save unknown labels: unknown/RAID-Dataset/
    if len(df_unknown) > 0:
        unknown_dir = output_dir / 'unknown' / 'RAID-Dataset'
...
class ChunkProcessor:
    ...
    def process_csv_in_chunks(self) -> Dict[str, Any]:
        """
        Process CSV file in chunks with progress tracking.
